...
# calculate_activity_score <- function(graph_object){
#   dataframe <- as_long_data_frame(graph_object)
#   pas <- sum(c(dataframe$from_weight[1], dataframe$weight * dataframe$to_weight)) # weight=Edge_weight, to_weight=Vertex_weight
#   if(is.na(pas)==TRUE){
...
# permutation.graph.test <- function(receiver_all, collective_database, score, graph, kegg_df, n){
#   set.seed(500)
#   distribution <- c()
...
#   for(i in 2:n){
#     random_from <- sample(user_pathway$Vertex_weight, size = nrow(graph_dataframe_user))
#     random_to <- sample(user_pathway$Vertex_weight, size = nrow(graph_dataframe_user))
#     perm_user <- transform(graph_dataframe_user, from_weight = random_from, to_weight = random_to)
#     perm_enrich_score <- sum(perm_user$from_weight[1], perm_user$weight * perm_user$to_weight)
...
#   } else {
#     distribution <- sort(distribution, decreasing = FALSE)
#     dist_pos <- which(distribution == score)
#   }
#   value <- (dist_pos/n)
...
permutation.graph.test_v2 <- function(receiver_all, collective_database, score, graph, kegg_df, n, lambda){
  ...
  for(i in 2:n){
    # random_from <- sample(user_pathway$Vertex_weight, size = nrow(graph_dataframe_user))
    # random_to <- sample(user_pathway$Vertex_weight, size = nrow(graph_dataframe_user))
    # random_from <- sample(weights_dataframe$avg_log2FC* -log(as.double(weights_dataframe$p_val_adj)), size = nrow(graph_dataframe_user))
    # random_to <- sample(weights_dataframe$avg_log2FC* -log(as.double(weights_dataframe$p_val_adj)), size = nrow(graph_dataframe_user))
...
# 
# find_sig_of_pathway_branches <- function(graph_list,export_directory_path,collective_database,overall_dataframe, by="Branch", receiver_df, permutation_num=500){
#   dataframe_list <- split(overall_dataframe, f = overall_dataframe$Branch)
#   d <- 1
#   new_dataframe_list <- c()
#   for(graph in graph_list){
#     enrichment_score <- calculate_activity_score(graph)
#     subset_df <- as.data.frame(dataframe_list[d])
#     colnames(subset_df) <- c("From", "to", "Direction", "Type", "Pathway_name", "Database_source","Conc", "DB_edge_correlation", "DB_edge_color", "Branch", "Branch_path", "Signaling_protein")
#     p_val <- permutation.graph.test(receiver_df, collective_database, enrichment_score, graph, subset_df, permutation_num)
#     subset_df$p_val <- rep(p_val, nrow(subset_df))
#     new_dataframe_list[[d]] <- subset_df
#     # print(paste0(d, "/", length(graph_list), "done." ))
#     d <- d + 1
#   }
...
find_sig_of_pathway_branches_v2 <- function(graph_list,export_directory_path,collective_database, overall_dataframe, receiver_df, permutation_num=500, lambda=1){
  ...
  for(graph in graph_list){
    ...
    # subset_df <- as.data.frame(dataframe_list[d])
    # subset_df <- as.data.frame(overall_dataframe[[d]])
    # colnames(subset_df) <- c("From", "to", "Direction", "Type", "Pathway_name", "Database_source","Conc", "DB_edge_correlation", "DB_edge_color", "Branch", "Branch_path", "Signaling_protein", "Global_sign", "receiver_avg_log2FC", "Edge_weight")
...
###################### RESULTS FOUND #################################
#write.table(final_dataframe, paste0(export_directory_path, "/", specified_sender, "_", specified_receiver, "/significant_branches.txt"), sep = '\t', quote = FALSE, row.names = FALSE)
