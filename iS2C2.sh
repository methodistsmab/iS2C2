...
# Supports both Ollama (local) and Gemini API (cloud) for LLM analysis
...
CONTEXT_SIZE="131072"
SEED="512"
API_KEY=""  # Required for Gemini API
ALGORITHM='s2c2'

# File paths
S2C2_SCRIPT="S2C2_CLI.R"
OLLAMA_SCRIPT="local-ollama-api.py"
GEMINI_SCRIPT="gemini-api-call.py"
...
LLM CONFIGURATION:
    --llm-provider STRING          LLM provider: "ollama" or "gemini" (default: ollama)
    --model STRING                 Model name (default: llama3.2 for ollama, gemini-2.0-flash for gemini)
    --api-key STRING               API key (required for Gemini API)
    --disease-context STRING       Disease context for LLM analysis (default: "Alzheimer's disease")
    --algorithm STRING             Algorithm for LLM hypothesis generation (default: s2c2)
...
OPTIONAL PARAMETERS:
    ...
    --max-tokens INT               Maximum tokens for LLM generation (default: 1500)
    --context-size INT             Context window size for LLM (default: 131072)
    --seed INT                     Random seed for LLM (default: 512)

OTHER OPTIONS:
    --s2c2-script FILE             Path to S2C2_CLI.R script (default: S2C2_CLI.R)
    --ollama-script FILE           Path to local-ollama.py script (default: local-ollama.py)
    --gemini-script FILE           Path to gemini-api-call.py script (default: gemini-api-call.py)
...
EXAMPLES:
    ...
    $0 --rds-file ./Ostesarcoma_Reduced_celltype.rds \
       --celltype-colname "Cell.Types" \
       --condition-colname "condition" \
       --condition1 "tumor" \
       --condition2 "NA" \
...
if [[ "$LLM_PROVIDER" != "ollama" && "$LLM_PROVIDER" != "gemini" ]]; then
    print_error "LLM provider must be 'ollama' or 'gemini', got: $LLM_PROVIDER"
    exit 1
fi
...
# Validate sender and receiver are not empty
if [ -z "$SENDER" ]; then
    print_error "Sender is empty"
    exit 1
fi

if [ -z "$RECEIVER" ]; then
    print_error "Receiver is empty"
    exit 1
fi
...
# Ensure ALGORITHM is set (default to s2c2 if not set)
if [ -z "$ALGORITHM" ]; then
    ALGORITHM="s2c2"
fi
