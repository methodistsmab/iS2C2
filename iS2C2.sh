...
OPENROUTER_SCRIPT="openrouter-api-call.py"
...
OPENROUTER SPECIFIC PARAMETERS:
    --site-url STRING              Site URL for OpenRouter rankings (optional)
    --site-name STRING             Site name for OpenRouter rankings (optional)
...
OTHER OPTIONS:
    ...
    --openrouter-script FILE       Path to openrouter-api-call.py script (default: openrouter-api-call.py)
...
if [[ "$LLM_PROVIDER" == "ollama" ]]; then
    ...
    if [ -n "$SITE_URL" ]; then
        OPENROUTER_BASE_CMD="$OPENROUTER_BASE_CMD --site-url \"$SITE_URL\""
    fi
    
    if [ -n "$SITE_NAME" ]; then
        OPENROUTER_BASE_CMD="$OPENROUTER_BASE_CMD --site-name \"$SITE_NAME\""
    fi
