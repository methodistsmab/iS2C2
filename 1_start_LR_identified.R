...
find_ligand_receptor_pairs <- function(seurat_object, markers_df, specified_sender, specified_receiver, sender_df, receiver_df, LR_database, condition_colname, condition1, condition2 = NA){
  # sender_seurat <- subset(seurat_object, idents = specified_sender)
  # receiver_seurat <- subset(seurat_object, idents = specified_receiver)
  # subset_seurat <- subset(seurat_object, idents = c(specified_sender, specified_receiver)) # added so that findmarker has 2 groups for comparison
